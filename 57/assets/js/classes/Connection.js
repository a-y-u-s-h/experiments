class Connection {
  ...
  display() {
    ...
    if (this.sending) {
      fill(0);
      strokeWeight(1);
      ellipse(this.dataflow.x, this.dataflow.y, 16, 16);
    }
    pop();
  }

  feedforward(input) {
    this.dataflow = new p5.Vector(this.source.location.x, this.source.location.y); // Start animation at Source Neuron
